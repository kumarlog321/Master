#include "aruco.h"
#include "cvdrawingutils.h"

#include <fstream>
#include <iostream>
#include <sstream>
#include <string>
#include <stdexcept>

#include <opencv2/highgui/highgui.hpp>
#include <opencv2/imgproc/imgproc.hpp>
#include <opencv2/imgproc.hpp>
#include <opencv2/calib3d.hpp>
//#include "aruco_test_thesis.h"
#include <assert.h>

cv::Mat Rx(float angle)
{
	/* Calculate rotation about x axis */
	cv::Mat R_x = (cv::Mat_<float>(3, 3) <<
		1, 0, 0,
		0, cos(angle), -sin(angle),
		0, sin(angle), cos(angle)
		);
	return R_x;
}

cv::Mat Ry(float angle)
{
	/* Calculate rotation about y axis */
	cv::Mat R_y = (cv::Mat_<float>(3, 3) <<
		cos(angle), 0, sin(angle),
		0, 1, 0,
		-sin(angle), 0, cos(angle)
		);
	return R_y;
}

cv::Mat Rz(float angle)
{
	/* Calculate rotation about z axis */
	cv::Mat R_z = (cv::Mat_<float>(3, 3) <<
		cos(angle), -sin(angle), 0,
		sin(angle), cos(angle), 0,
		0, 0, 1);
	return R_z;
}


float deg2rad(float theta)
{
	return (theta * 0.01745329251994329576923690768489);
}

cv::Mat setR(float angle_x, float angle_y, float angle_z)
{
	return Rx(deg2rad(angle_x)) * Ry(deg2rad(angle_y)) * Rz(deg2rad(angle_z));
}

struct   TimerAvrg {
	std::vector<double> times;
	size_t curr = 0, n; std::chrono::high_resolution_clock::time_point begin, end;   TimerAvrg(int _n = 30)
	{
		n = _n; times.reserve(n);
	}inline void start()
	{
		begin = std::chrono::high_resolution_clock::now();
	}inline void stop()
	{
		end = std::chrono::high_resolution_clock::now(); double duration = double(std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count())*1e-6; if (times.size() < n) times.push_back(duration); else { times[curr] = duration; curr++; if (curr >= times.size()) curr = 0; }
	}double getAvrg()
	{
		double sum = 0; for (auto t : times) sum += t; return sum / double(times.size());
	}
};

/* create 4x4 matrix float */
cv::Mat T_euler(float x, float y, float z, float rx, float ry, float rz);
float e_dist(cv::Mat T4x4, float gx, float gy, float gz);
float e_dist_T(cv::Mat T4x4, cv::Mat gtT4x4);
void QuatsToMat(const float *q, float *m);
cv::Mat T_from_q(float x, float y, float z, float qw, float qx, float qy, float qz);
float error(cv::Mat);
float errorPositining(cv::Mat);

cv::Mat rot2euler(cv::Mat dest);
cv::Mat convert4x4to3x3(cv::Mat T);
cv::Vec3f rotationMatrixToEulerAngles(cv::Mat cam0_id113x3);
cv::Mat rot_to_euler(cv::Mat R);
//void rotationMatrixToEulerAngles(cv::Mat dest);



int main(int argc, char** argv)
{
	TimerAvrg fps_;
	std::string dictionaryStr_00 = "ALL_DICTS";
	std::string dictionaryStr_01 = "ALL_DICTS";
	std::string dictionaryStr_1 = "ALL_DICTS";
	float markerSize_00 = 0.20f; // marker on the car
	float markerSize_01 = 0.20f; // external marker
	float markerSize_1 = 0.20f;
	aruco::CameraParameters camparams_0;
	aruco::CameraParameters camparams_1;
	aruco::MarkerDetector detector_00;
	aruco::MarkerDetector detector_01;
	aruco::MarkerDetector detector_1;
	camparams_0.readFromXMLFile("cam_0.yml");
	//camparams_0.CameraMatrix.at<float>(0,2) += 4.0f;
	camparams_1.readFromXMLFile("cam_1.yml");
	std::ofstream myfile;
	myfile.open("example1.csv");
	std::ofstream myfiler;
	myfiler.open("rotational error.csv");
	std::ofstream myfilee;
	myfilee.open("Euclidean error.csv");
	std::ofstream myfilex;
	myfilex.open("error in x.csv");
	std::ofstream myfiley;
	myfiley.open("error in y.csv");
	std::ofstream myfilez;
	myfilez.open("error in z.csv");
	std::ofstream myfileroll;
	myfileroll.open("error in roll.csv");
	std::ofstream myfilepitch;
	myfilepitch.open("error in pitch.csv");
	std::ofstream myfileyaw;
	myfileyaw.open("error in yaw.csv");

	myfiler << "Rotational error" << "\n" << "," << "," << "," << "id11_id17," << "id17_cam1," << "id17_cam0," << "id11_cam0," << "Cam1_VCS," << "\n";
	myfilee << "Euclidean error" << "\n"<< "," << "," << "," << "id11_id17," << "id17_cam1," << "id17_cam0," << "id11_cam0," << "Cam1_VCS," << "\n";
	
	myfileroll << "Roll error" << "\n" << "," << "," << "," << "id11_id17," << "id17_cam1," << "id17_cam0," << "id11_cam0," << "Cam1_VCS," << "\n";
	myfilepitch << "Pitch error" << "\n" << "," << "," << "," << "id11_id17," << "id17_cam1," << "id17_cam0," << "id11_cam0," << "Cam1_VCS," << "\n";
	myfileyaw << "Yaw error" << "\n" << "," << "," << "," <<"id11_id17," << "id17_cam1," << "id17_cam0," << "id11_cam0," << "Cam1_VCS," << "\n";

	myfilez << "z error" << "\n" << "," << "," << "," << "id11_id17," << "id17_cam1," << "id17_cam0," << "id11_cam0," << "Cam1_VCS," << "\n";
	myfilex << "x error" << "\n" << "," << "," << "," << "id11_id17," << "id17_cam1," << "id17_cam0," << "id11_cam0," << "Cam1_VCS," << "\n";
	myfiley << "y error" << "\n" << "," << "," << "," << "id11_id17," << "id17_cam1," << "id17_cam0," << "id11_cam0," << "Cam1_VCS," << "\n";

	float cam_px = camparams_0.CameraMatrix.at<float>(0, 2);
	float cam_py = camparams_0.CameraMatrix.at<float>(1, 2);
	float cam_fx = camparams_0.CameraMatrix.at<float>(0, 0);
	float cam_fy = camparams_0.CameraMatrix.at<float>(1, 1);

	cv::Mat image_0org;
	cv::Mat image_1org;
	image_0org = cv::imread("renderimage0_11_17(20).png", 1);
	image_1org = cv::imread("renderimage1_11_17(20).png", 1);
#define DEBUGVERSION 0

	float maxerror = 5;
	float niteration = 100;
	for (float xrand = 0.1; xrand < maxerror; xrand = xrand + 0.2) {
	
		//std::cout << std::endl;
		//std::cout << "xrand: " << xrand << std::endl;
		float sumerror_px = 0.0f;
		float sumerror_id11_id17 = 0.0f;
		float sum_euler_error_id17_cam1_00 = 0.0f;
		float sum_euler_error_id17_cam1_10 = 0.0f;
		float sum_euler_error_id17_cam1_20 = 0.0f;

		cv::Mat sumeuler_error_id11_id17;

		float sumeuler_error_id11_id17_10 = 0.0f;

		float sumerror_euclidean_id11_id17 = 0.0f;
		float sumeuler_error_id11_id17_00 = 0.0f;


		float sumeuler_error_id11_id17_20 = 0.0f;
		float sumT_error_id11_id17_03 = 0.0f;
		float sumT_error_id11_id17_13 = 0.0f;
		float sumT_error_id11_id17_23 = 0.0f;

		float sumT_error_cam1_vcs_03 = 0.0f;
		float sumT_error_cam1_vcs_13 = 0.0f;
		float sumT_error_cam1_vcs_23 = 0.0f;

		float sumeuler_error_cam1_vcs_00 = 0.0f;
		float sumeuler_error_cam1_vcs_10 = 0.0f;
		float sumeuler_error_cam1_vcs_20 = 0.0f;

		float sumerror_id17_cam1 = 0.0f;
		float sumerror_euclidean_id17_cam1 = 0.0f;


		float sumT_error_id17_cam1_03 = 0.0f;
		float sumT_error_id17_cam1_13 = 0.0f;
		float sumT_error_id17_cam1_23 = 0.0f;

		float sumerror_id17_cam0 = 0.0f;
		float sumerror_euclidean_id17_cam0 = 0.0f;
		float sumeuler_error_id17_cam0_00 = 0.0f;
		float sumeuler_error_id17_cam0_10 = 0.0f;
		float sumeuler_error_id17_cam0_20 = 0.0f;

		float sumT_error_id17_cam0_03 = 0.0f;
		float sumT_error_id17_cam0_13 = 0.0f;
		float sumT_error_id17_cam0_23 = 0.0f;

		float sumT_error_id11_cam0 = 0.0f;

		float sumerror_id11_cam0 = 0.0f;
		float sumerror_euclidean_id11_cam0 = 0.0f;
		float sumeuler_error_id11_cam0_00 = 0.0f;
		float sumeuler_error_id11_cam0_10 = 0.0f;
		float sumeuler_error_id11_cam0_20 = 0.0f;

		float sumT_error_id11_cam0_03 = 0.0f;
		float sumT_error_id11_cam0_13 = 0.0f;
		float sumT_error_id11_cam0_23 = 0.0f;

		float sumerror_R_Cam1_VCS = 0.0f;
		float sumerror_euclidean_cam1_vcs = 0.0f;

		for (int iteration = 0; iteration < niteration; ++iteration) {

		//int iteration = 1;
			float randvalue = (static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (2.0f * xrand)))) - xrand;
			//std::cout << "randvalue=" << randvalue << std::endl;
			//float original_cam_px =camparams_0.CameraMatrix.at<float>(0, 2);

			//camparams_0.CameraMatrix.at<float>(0, 2) = cam_px + randvalue;
			//std::cout << "changed value =" << camparams_0.CameraMatrix.at<float>(0, 2) << "\n";
			//camparams_0.CameraMatrix.at<float>(1, 2) = cam_py + randvalue;
			camparams_0.CameraMatrix.at<float>(0, 0) = cam_fx + randvalue;
			camparams_0.CameraMatrix.at<float>(1, 1) = cam_fy + randvalue;

			detector_00.setDictionary(dictionaryStr_00, 0.0f);
			detector_01.setDictionary(dictionaryStr_01, 0.0f);
			detector_1.setDictionary(dictionaryStr_1, 0.0f);
			//image0 is read from external camera
			cv::Mat image_0;
			cv::Mat image_1;
			//image_0 = cv::imread("renderimage0_11_17(27).png", 1);
			//image_1 = cv::imread("renderimage1_11_17(27).png", 1);
			image_0 = image_0org.clone();
			image_1 = image_1org.clone();
			std::vector<aruco::Marker> markers_00;
			std::vector<aruco::Marker> markers_01;
			std::vector<aruco::Marker> markers_1;

			fps_.start();
			markers_00 = detector_00.detect(image_0, camparams_0, markerSize_00);
			markers_01 = detector_01.detect(image_0, camparams_0, markerSize_01);
			markers_1 = detector_1.detect(image_1, camparams_1, markerSize_1);
			fps_.stop();
#if DEBUGVERSION
			/* check the speed by calculating the mean speed of all iterations */
			std::cout << "\rTime detection=" << fps_.getAvrg() * 1000 << " milliseconds " << std::endl;
			std::cout << "\r nmarkers = " << markers_00.size() << " images resolution = " << image_0.size() << std::endl;
			std::cout << "\r nmarkers = " << markers_01.size() << " images resolution = " << image_0.size() << std::endl;
			std::cout << "\r nmarkers = " << markers_1.size() << " images resolution = " << image_1.size() << std::endl;


			/* The first camera */
			auto candidates_0 = detector_00.getCandidates();
			for (auto cand : candidates_0)
				aruco::Marker(cand, -1).draw(image_0, cv::Scalar(255, 0, 255));

			for (unsigned int i = 0; i < markers_00.size(); i++) {
				std::cout << markers_00[i] << std::endl;
				markers_00[i].draw(image_0, cv::Scalar(0, 0, 255), 2, true);
			}

			/* draw a 3d cube in each marker if there is 3d info */
			if (camparams_0.isValid() && markerSize_00 > 0) {
				for (unsigned int i = 0; i < markers_00.size(); i++) {
					aruco::CvDrawingUtils::draw3dCube(image_0, markers_00[i], camparams_0);
					aruco::CvDrawingUtils::draw3dAxis(image_0, markers_00[i], camparams_0);
				}
			}

			/* The second camera */
			auto candidates_1 = detector_1.getCandidates();
			for (auto cand : candidates_1)
				aruco::Marker(cand, -1).draw(image_1, cv::Scalar(255, 0, 255));

			for (unsigned int i = 0; i < markers_1.size(); i++) {
				std::cout << markers_1[i] << std::endl;
				markers_1[i].draw(image_1, cv::Scalar(0, 0, 255), 2, true);
			}

			/* draw a 3d cube in each marker if there is 3d info */
			if (camparams_1.isValid() && markerSize_1 > 0) {
				for (unsigned int i = 0; i < markers_1.size(); i++) {
					aruco::CvDrawingUtils::draw3dCube(image_1, markers_1[i], camparams_1);
					aruco::CvDrawingUtils::draw3dAxis(image_1, markers_1[i], camparams_1);
				}
			}
#endif
			/* TODO: check if detected */
			cv::Mat T_id11_cam0; //= markers_00[0].getTransformMatrix();
			cv::Mat T_id17_cam0; // = markers_00[1].getTransformMatrix();

			if (markers_00[0].id == 11) {
				T_id11_cam0 = markers_00[0].getTransformMatrix();
			}
			else {
				T_id11_cam0 = markers_00[1].getTransformMatrix();
			}

			if (markers_01[0].id == 17) {
				T_id17_cam0 = markers_01[0].getTransformMatrix();
			}
			else {
				T_id17_cam0 = markers_01[1].getTransformMatrix();
			}

			cv::Mat T_id17_cam1 = markers_1[0].getTransformMatrix();

			cv::Mat T_cam0_id11 = T_id11_cam0.inv();
			cv::Mat T_cam0_id17 = T_id17_cam0.inv();
			cv::Mat T_cam1_id17 = T_id17_cam1.inv();

			/* cam0 External */
			float gx_cam0 = 0.233906;
			float gy_cam0 = 2.44256;
			float gz_cam0 = -0.152881;
			//float g_roll_cam0 = -80.7258f;
			//float g_pitch_cam0 = 179.964f;
			//float g_yaw_cam0 = 269.0f;
			cv::Mat Tcam0 = T_from_q(gx_cam0, gy_cam0, gz_cam0, 0.015304f, 0.017521f, 0.761756f, 0.647446f) * T_euler(0, 0, 0, 180, 0, 0);

			/* cam1*/
			float gx_cam1 = -1.58617f;
			float gy_cam1 = -1.00329f;
			float gz_cam1 = 0.807367f;
			//float g_roll_cam1 = -9.96f;
			//float g_pitch_cam1 = 91.2f;
			//float g_yaw_cam1 = 87.2f;
			cv::Mat Tcam1 = T_from_q(gx_cam1, gy_cam1, gz_cam1, -0.564695f, -0.410791f, 0.509194f, 0.503083f) * T_euler(0, 0, 0, 180, 0, 0);

			/* Id17 - the goal and GT*/
			float gx_id17 = 0.645195f;
			float gy_id17 = -0.855396f;
			float gz_id17 = 0.523349f;
			//float g_roll_id17 = -45;
			//float g_pitch_id17 = 0.0f;
			//float g_yaw_id17 = 0.0f;
			cv::Mat Tid17 = T_from_q(gx_id17, gy_id17, gz_id17, 0.229767f, -0.659454f, 0.340739f, 0.629464f);
			//cv::Mat Tid17_ = T_euler(gx_id17, gy_id17, gz_id17, g_roll_id17, g_pitch_id17, g_yaw_id17);

			/* Id11 */
			float gx_id11 = -0.47856f;
			float gy_id11 = -0.683885f;
			float gz_id11 = 0.289345f;
			//float g_roll_id11 = 0.0f;
			//float g_pitch_id11 = 0.0f;
			//float g_yaw_id11 = 0.0f;
			cv::Mat Tid11 = T_from_q(gx_id11, gy_id11, gz_id11, 0.464639f, -0.521328f, 0.555687f, 0.451154f);

			cv::Mat T_cam0_id17_gt = Tid17.inv() * Tcam0;
			cv::Mat T_cam0_id11_gt = Tid11.inv() * Tcam0;
			cv::Mat T_cam1_id17_gt = Tid17.inv() * Tcam1;
			cv::Mat T_id17_id11_gt = Tid11.inv() * Tid17;
			cv::Mat T_id11_id17_gt = Tid17.inv() * Tid11;

			cv::Mat T_id17_id11_sensor = T_id11_cam0.inv() * T_id17_cam0;
			cv::Mat T_id11_id17_sensor = T_id17_cam0.inv() * T_id11_cam0;

			/* This is the concept */
			cv::Mat TmarkerOnTheCar = Tid11;
			cv::Mat Tcam12vcs = TmarkerOnTheCar * T_id11_cam0.inv() * T_id17_cam0 * T_id17_cam1.inv(); //the goal
			cv::Mat Tcam02vcs = TmarkerOnTheCar * T_id11_cam0.inv();
			cv::Mat Tid172vcs = TmarkerOnTheCar * T_id11_cam0.inv() * T_id17_cam0;


			// Error anaylsis
			cv::Mat T_error_id11_id17 = T_id11_id17_gt.inv() * T_id11_id17_sensor;

			cv::Mat T_error_id17_cam1 = T_id17_cam1 * T_cam1_id17_gt;

			cv::Mat T_error_id17_cam0 = T_id17_cam0 * T_cam0_id17_gt;

			cv::Mat T_error_id11_cam0 = T_id11_cam0 * T_cam0_id11_gt;



			float error_id11_id17 = error(T_error_id11_id17);
			float error_id17_cam1 = error(T_error_id17_cam1);
			float error_id17_cam0 = error(T_error_id17_cam0);
			float error_id11_cam0 = error(T_error_id11_cam0);

			float error_euclidean_id11_id17 = errorPositining(T_error_id11_id17);
			float error_euclidean_id17_cam1 = errorPositining(T_error_id17_cam1);
			float error_euclidean_id17_cam0 = errorPositining(T_error_id17_cam0);
			float error_euclidean_id11_cam0 = errorPositining(T_error_id11_cam0);


			//float error1 = error(T_error_id11_cam0);

			cv::Mat T_error_cam1_vcs = Tcam1.inv() * Tcam12vcs; // Final KPI
			float error_R_Cam1_VCS = error(T_error_cam1_vcs);
			//float error_R_Cam1_VCS = std::abs(T_error_cam1_vcs.at<float>(0, 0) - 1.0f) + std::abs(T_error_cam1_vcs.at<float>(1, 1) - 1.0f) + std::abs(T_error_cam1_vcs.at<float>(2, 2) - 1.0f);
			//positioning
			float error_euclidean_cam1_vcs = errorPositining(T_error_cam1_vcs);
			//float error_euclidean_cam1_vcs = std::sqrt(std::powf(T_error_cam1_vcs.at<float>(0, 3), 2) + std::powf(T_error_cam1_vcs.at<float>(1, 3), 2) + std::powf(T_error_cam1_vcs.at<float>(2, 3), 2));





			cv::Mat euler_error_id11_id17 = rot_to_euler(T_error_id11_id17);
			cv::Mat euler_error_id17_cam1 = rot_to_euler(T_error_id17_cam1);
			cv::Mat euler_error_id17_cam0 = rot_to_euler(T_error_id17_cam0);
			cv::Mat euler_error_id11_cam0 = rot_to_euler(T_error_id11_cam0);

			cv::Mat euler_error_cam1_vcs = rot_to_euler(T_error_cam1_vcs);

			float error_px = 0.0;
			error_px = randvalue;
			sumerror_px += error_px;
			sumerror_id11_id17 += error_id11_id17;
			sumerror_euclidean_id11_id17 += error_euclidean_id11_id17;

			//sumeuler_error_id11_id17 += euler_error_id11_id17;

			sumeuler_error_id11_id17_00 += euler_error_id11_id17.at<float>(0, 0);
			sumeuler_error_id11_id17_10 += euler_error_id11_id17.at<float>(1, 0);
			sumeuler_error_id11_id17_20 += euler_error_id11_id17.at<float>(2, 0);

			sumT_error_id11_id17_03 += T_error_id11_id17.at<float>(0, 3);
			sumT_error_id11_id17_13 += T_error_id11_id17.at<float>(1, 3);
			sumT_error_id11_id17_23 += T_error_id11_id17.at<float>(2, 3);

			sumT_error_cam1_vcs_03 += T_error_cam1_vcs.at<float>(0, 3);
			sumT_error_cam1_vcs_13 += T_error_cam1_vcs.at<float>(1, 3);
			sumT_error_cam1_vcs_23 += T_error_cam1_vcs.at<float>(2, 3);

			sumeuler_error_cam1_vcs_00 += euler_error_cam1_vcs.at<float>(0, 0);
			sumeuler_error_cam1_vcs_10 += euler_error_cam1_vcs.at<float>(1, 0);
			sumeuler_error_cam1_vcs_20 += euler_error_cam1_vcs.at<float>(2, 0);

			sumerror_id17_cam1 += error_id17_cam1;
			sumerror_euclidean_id17_cam1 += error_euclidean_id17_cam1;


			sumT_error_id17_cam1_03 += T_error_id17_cam1.at<float>(0, 3);
			sumT_error_id17_cam1_13 += T_error_id17_cam1.at<float>(1, 3);
			sumT_error_id17_cam1_23 += T_error_id17_cam1.at<float>(2, 3);

			sumerror_id17_cam0 += error_id17_cam0;
			sumerror_euclidean_id17_cam0 += error_euclidean_id17_cam0;
			sumeuler_error_id17_cam0_00 += euler_error_id17_cam0.at<float>(0, 0);
			sumeuler_error_id17_cam0_10 += euler_error_id17_cam0.at<float>(1, 0);
			sumeuler_error_id17_cam0_20 += euler_error_id17_cam0.at<float>(2, 0);

			sumT_error_id17_cam0_03 += T_error_id17_cam0.at<float>(0, 3);
			sumT_error_id17_cam0_13 += T_error_id17_cam0.at<float>(1, 3);
			sumT_error_id17_cam0_23 += T_error_id17_cam0.at<float>(2, 3);

			sumerror_id11_cam0 += error_id11_cam0;
			sumerror_euclidean_id11_cam0 += error_euclidean_id11_cam0;
			sumeuler_error_id11_cam0_00 += euler_error_id11_cam0.at<float>(0, 0);
			sumeuler_error_id11_cam0_10 += euler_error_id11_cam0.at<float>(1, 0);
			sumeuler_error_id11_cam0_20 += euler_error_id11_cam0.at<float>(2, 0);

			sumT_error_id11_cam0_03 += T_error_id11_cam0.at<float>(0, 3);
			sumT_error_id11_cam0_13 += T_error_id11_cam0.at<float>(1, 3);
			sumT_error_id11_cam0_23 += T_error_id11_cam0.at<float>(2, 3);

			sum_euler_error_id17_cam1_00 += euler_error_id17_cam1.at<float>(0, 0);
			sum_euler_error_id17_cam1_10 += euler_error_id17_cam1.at<float>(1, 0);
			sum_euler_error_id17_cam1_20 += euler_error_id17_cam1.at<float>(2, 0);

			sumerror_R_Cam1_VCS += error_R_Cam1_VCS;
			sumerror_euclidean_cam1_vcs += error_euclidean_cam1_vcs;

			 //camparams_0.CameraMatrix.at<float>(0, 2) = original_cam_px ;
			 //std::cout << "original value =" << camparams_0.CameraMatrix.at<float>(0, 2) << "\n";
		}
		


		float avg_error_id11_id17 = sumerror_id11_id17/( niteration) ;
		float avg_error_euclidean_id11_id17 = sumerror_euclidean_id11_id17 / (niteration );



		float avg_euler_error_id11_id17_00 = sumeuler_error_id11_id17_00 / (niteration );
		float avg_euler_error_id11_id17_10 = sumeuler_error_id11_id17_10 / (niteration );
		float avg_euler_error_id11_id17_20 = sumeuler_error_id11_id17_20 / (niteration );
		float avg_euler_error_id17_cam1_00 = sum_euler_error_id17_cam1_00 / (niteration);
		float avg_euler_error_id17_cam1_10 = sum_euler_error_id17_cam1_10 / (niteration);
		float avg_euler_error_id17_cam1_20 = sum_euler_error_id17_cam1_20 / (niteration);


		float avg_T_error_cam1_vcs_03 = sumT_error_cam1_vcs_03 / (niteration );
		float avg_T_error_cam1_vcs_13 = sumT_error_cam1_vcs_13 / (niteration );
		float avg_T_error_cam1_vcs_23 = sumT_error_cam1_vcs_23 / (niteration );
		float avg_euler_error_cam1_vcs_00 = sumeuler_error_cam1_vcs_00 / (niteration);
		float avg_euler_error_cam1_vcs_10 = sumeuler_error_cam1_vcs_10 / (niteration);
		float avg_euler_error_cam1_vcs_20 = sumeuler_error_cam1_vcs_20 / (niteration);
		float avg_T_error_id11_id17_03 = sumT_error_id11_id17_03 / (niteration );
		float avg_T_error_id11_id17_13 = sumT_error_id11_id17_13 / (niteration );
		float avg_T_error_id11_id17_23 = sumT_error_id11_id17_23 / (niteration );
		float avg_error_id17_cam1 = sumerror_id17_cam1 / (niteration );
		float avg_error_euclidean_id17_cam1 = sumerror_euclidean_id17_cam1 / (niteration );

		float avg_T_error_id17_cam1_03 = sumT_error_id17_cam1_03 / (niteration );//
		float avg_T_error_id17_cam1_13 = sumT_error_id17_cam1_13 / (niteration);
		float avg_T_error_id17_cam1_23 = sumT_error_id17_cam1_23 / (niteration );
		float avg_error_id17_cam0 = sumerror_id17_cam0 / (niteration/1);
		float avg_error_euclidean_id17_cam0 = sumerror_euclidean_id17_cam0 / (niteration );
		float avg_euler_error_id17_cam0_00 = sumeuler_error_id17_cam0_00 / (niteration);
		float avg_euler_error_id17_cam0_10 = sumeuler_error_id17_cam0_10 / (niteration);
		float avg_euler_error_id17_cam0_20 = sumeuler_error_id17_cam0_20 / (niteration);
		float avg_T_error_id17_cam0_03 = sumT_error_id17_cam0_03 / (niteration );
		float avg_T_error_id17_cam0_13 = sumT_error_id17_cam0_13 / (niteration );
		float avg_T_error_id17_cam0_23 = sumT_error_id17_cam0_23 / (niteration);
		float avg_T_error_id11_cam0 = sumT_error_id11_cam0 / (niteration );
		float avg_error_id11_cam0 = sumerror_id11_cam0 / (niteration);
		float avg_error_euclidean_id11_cam0 = sumerror_euclidean_id11_cam0 / (niteration);
		float avg_euler_error_id11_cam0_00 = sumeuler_error_id11_cam0_00 / (niteration);
		float avg_euler_error_id11_cam0_10 = sumeuler_error_id11_cam0_10 / (niteration);
		float avg_euler_error_id11_cam0_20 = sumeuler_error_id11_cam0_20 / (niteration);
		float avg_T_error_id11_cam0_03 = sumT_error_id11_cam0_03 / (niteration);
		float avg_T_error_id11_cam0_13 = sumT_error_id11_cam0_13 / (niteration);
		float avg_T_error_id11_cam0_23 = sumT_error_id11_cam0_23 / (niteration);
		float avg_error_R_Cam1_VCS = sumerror_R_Cam1_VCS / (niteration );
		float avg_error_euclidean_cam1_vcs = sumerror_euclidean_cam1_vcs / (niteration );

		//std::cout << avg_error_px << std::endl;





		myfiler << "x0=" << xrand    << ","<< camparams_0.CameraMatrix.at<float>(0, 2)<< ",";
		myfilee << "x0=" << xrand    << ","<< camparams_0.CameraMatrix.at<float>(0, 2)<< ",";
		myfilex << "x0=" << xrand    << ","<< camparams_0.CameraMatrix.at<float>(0, 2)<< ",";
		myfiley << "x0=" << xrand    << ","<< camparams_0.CameraMatrix.at<float>(0, 2)<< ",";
		myfilez << "x0=" << xrand    << ","<< camparams_0.CameraMatrix.at<float>(0, 2)<< ",";
		myfileroll << "x0=" << xrand << ","<< camparams_0.CameraMatrix.at<float>(0, 2)<< ",";
		myfilepitch << "x0=" << xrand<< ","<< camparams_0.CameraMatrix.at<float>(0, 2)<< ",";
		myfileyaw << "x0="  << xrand << ","<< camparams_0.CameraMatrix.at<float>(0, 2)<< ",";




		//std::ofstream myfile;
		//myfile.open("example.csv");
		//std::ofstream myfile("example.csv", std::ios_base::app | std::ios_base::out);

		myfile << camparams_0.CameraMatrix.at<float>(0, 0) << "," << camparams_0.CameraMatrix.at<float>(0, 1) << "," << camparams_0.CameraMatrix.at<float>(0, 2) << "\n";
		myfile << camparams_0.CameraMatrix.at<float>(1, 0) << "," << camparams_0.CameraMatrix.at<float>(1, 1) << "," << camparams_0.CameraMatrix.at<float>(1, 2) << "\n";
		myfile << camparams_0.CameraMatrix.at<float>(2, 0) << "," << camparams_0.CameraMatrix.at<float>(2, 1) << "," << camparams_0.CameraMatrix.at<float>(2, 2) << "\n\n";

		myfile << " ,Rotation error,Euclidean error,error in roll,error in pitch, error in yaw, error in x, error in y, error in z\n";
		myfile << "id11_id17," << avg_error_id11_id17 << "," << avg_error_euclidean_id11_id17 << "," << avg_euler_error_id11_id17_00 << "," << avg_euler_error_id11_id17_10 << "," << avg_euler_error_id11_id17_20 << "," << avg_T_error_id11_id17_03 << "," << avg_T_error_id11_id17_13 << "," << avg_T_error_id11_id17_23 << "\n";
		myfile << "id17_cam1," << avg_error_id17_cam1 << "," << avg_error_euclidean_id17_cam1 << "," << avg_euler_error_id17_cam1_00 << "," << avg_euler_error_id17_cam1_10 << "," << avg_euler_error_id17_cam1_20 << "," << avg_T_error_id17_cam1_03 << "," << avg_T_error_id17_cam1_13 << "," << avg_T_error_id17_cam1_23 << "\n";
		myfile << "id17_cam0," << avg_error_id17_cam0 << "," << avg_error_euclidean_id17_cam0 << "," << avg_euler_error_id17_cam0_00 << "," << avg_euler_error_id17_cam0_10 << "," << avg_euler_error_id17_cam0_20 << "," << avg_T_error_id17_cam0_03 << "," << avg_T_error_id17_cam0_13 << "," << avg_T_error_id17_cam0_23 << "\n";
		myfile << "id11_cam0," << avg_error_id11_cam0 << "," << avg_error_euclidean_id11_cam0 << "," << avg_euler_error_id11_cam0_00 << "," << avg_euler_error_id11_cam0_10 << "," << avg_euler_error_id11_cam0_20 << "," << avg_T_error_id11_cam0_03 << "," << avg_T_error_id11_cam0_13 << "," << avg_T_error_id11_cam0_23 << "\n";
		myfile << "Cam1_VCS," << avg_error_R_Cam1_VCS << "," << avg_error_euclidean_cam1_vcs << "," << avg_euler_error_cam1_vcs_00 << "," <<   avg_euler_error_cam1_vcs_10 << "," <<  avg_euler_error_cam1_vcs_20 << "," <<  avg_T_error_cam1_vcs_03 << "," <<  avg_T_error_cam1_vcs_13 << "," <<  avg_T_error_cam1_vcs_23 << "\n";
						
		 myfiler << "" << ","<< "" << avg_error_id11_id17 << "," << avg_error_id17_cam1 << "," << avg_error_id17_cam0 << "," << avg_error_id11_cam0 << "," << avg_error_R_Cam1_VCS << "\n";
		 		 
		 myfilee << "" << ","<< "" << avg_error_euclidean_id11_id17 << "," << avg_error_euclidean_id17_cam1 << "," << avg_error_euclidean_id17_cam0 << "," << avg_error_euclidean_id11_cam0 << "," << avg_error_euclidean_cam1_vcs << "\n";
		 
		 myfileroll << "" << "" << "," << "" << avg_euler_error_id11_id17_00 << "," << avg_euler_error_id17_cam1_00 << "," << avg_euler_error_id17_cam0_00 << "," << avg_euler_error_id11_cam0_00 << "," << avg_euler_error_cam1_vcs_00 << "\n";
		 		 
		 myfilepitch << "" << "" << "," << "" << avg_euler_error_id11_id17_10 << "," << avg_euler_error_id17_cam1_10 << "," << avg_euler_error_id17_cam0_10 << "," << avg_euler_error_id11_cam0_10 << "," << avg_euler_error_cam1_vcs_10 << "\n";

		 myfileyaw << "" << "" << "," << "" << avg_euler_error_id11_id17_20 << "," << avg_euler_error_id17_cam1_20 << "," << avg_euler_error_id17_cam0_20 << "," << avg_euler_error_id11_cam0_20 << "," << avg_euler_error_cam1_vcs_20 << "\n";

		 myfilex << "" << "" << "," << "" << avg_T_error_id11_id17_03 << "," << avg_T_error_id17_cam1_03 << "," << avg_T_error_id17_cam0_03 << "," << avg_T_error_id11_cam0_03 << "," << avg_T_error_cam1_vcs_03 << "\n";
		 
		 myfiley << "" << "" << "," << "" << avg_T_error_id11_id17_13 << "," << avg_T_error_id17_cam1_13 << "," << avg_T_error_id17_cam0_13 << "," << avg_T_error_id11_cam0_13 << "," << avg_T_error_cam1_vcs_13 << "\n";
		 
		 myfilez << "" << "" << "," << "" << avg_T_error_id11_id17_23 << "," << avg_T_error_id17_cam1_23 << "," << avg_T_error_id17_cam0_23 << "," << avg_T_error_id11_cam0_23 << "," << avg_T_error_cam1_vcs_23 << "\n";


		// Example1: for your report
		cv::Mat test_gt = cv::Mat::eye(4, 4, CV_32FC1);
		cv::Mat test_sensor = cv::Mat::eye(4, 4, CV_32FC1);
		test_sensor.at<float>(0, 3) = 0.01;
		cv::Mat error = test_gt.inv() * test_sensor;

		// Example2: for your ground truth
		cv::Mat test_gt2 = cv::Mat::eye(4, 4, CV_32FC1);
		cv::Mat test_sensor2 = cv::Mat::eye(4, 4, CV_32FC1);
		test_sensor2.at<float>(0, 3) = 0.01;
		test_sensor2.at<float>(1, 3) = 0.02;
		test_sensor2 = test_sensor2 * T_euler(0, 0, 0, 2, 1, -1);
		cv::Mat error2 = test_gt.inv() * test_sensor2;
		cv::Mat euler_error2 = rot_to_euler(error2);

		// Remarks: there is a precision loss during conversions.
		// Rendering error: ground truth has some errors: we ignore it


		
	}
	myfile.close();
	myfiler.close();
	myfilee.close();
	myfilex.close();
	myfiley.close();
	myfilez.close();
	myfileroll.close();
	myfilepitch.close();
	myfileyaw.close();

	return 0;
}

float error(cv::Mat Er)
{
	float error1 = std::abs(Er.at<float>(0, 0) - 1.0f) + std::abs(Er.at<float>(1, 1) - 1.0f) + std::abs(Er.at<float>(2, 2) - 1.0f);
	return error1;

}

float errorPositining(cv::Mat Er)
{
	float error2 = std::sqrt(std::powf(Er.at<float>(0, 3), 2) + std::powf(Er.at<float>(1, 3), 2) + std::powf(Er.at<float>(2, 3), 2));
	return error2;

}

float togetaverage(float value, int niteration)
{
	float avgr = value  / (niteration * 10000);
	return avgr;

}

cv::Mat convert4x4to3x3(cv::Mat mat4x4)
{
	int width = mat4x4.cols;
	int height = mat4x4.rows;
	cv::Mat rota = cv::Mat(3, 3, CV_32FC1);
	for (int x = 0; x < height - 1; x++)
	{
		for (int y = 0; y < width - 1; y++)
		{

			rota.at<float>(x, y) = mat4x4.at<float>(x, y);

		}
	}
	return rota;
}

cv::Vec3f rotationMatrixToEulerAngles(cv::Mat R)
{
	float sy = sqrt(R.at<float>(0, 0) * R.at<float>(0, 0) + R.at<float>(1, 0) * R.at<float>(1, 0));

	bool singular = sy < 1e-6; // If
	float x, y, z;
	if (!singular) {
		x = atan2(R.at<float>(2, 1), R.at<float>(2, 2));
		y = atan2(-R.at<float>(2, 0), sy);
		z = atan2(R.at<float>(1, 0), R.at<float>(0, 0));
	}
	else {
		x = atan2(-R.at<float>(1, 2), R.at<float>(1, 1));
		y = atan2(-R.at<float>(2, 0), sy);
		z = 0;
	}

	return cv::Vec3f(x, y, z);
}

cv::Mat rot_to_euler(cv::Mat R)
{
	float sy = sqrt(R.at<float>(0, 0) * R.at<float>(0, 0) + R.at<float>(1, 0) * R.at<float>(1, 0));

	bool singular = sy < 1e-6; // If
	float x, y, z;
	if (!singular) {
		x = atan2(R.at<float>(2, 1), R.at<float>(2, 2));
		y = atan2(-R.at<float>(2, 0), sy);
		z = atan2(R.at<float>(1, 0), R.at<float>(0, 0));
	}
	else {
		x = atan2(-R.at<float>(1, 2), R.at<float>(1, 1));
		y = atan2(-R.at<float>(2, 0), sy);
		z = 0;
	}

	return cv::Mat(cv::Vec3f(x, y, z) * 180.0f / CV_PI);
}


cv::Mat T_euler(float x, float y, float z, float rx, float ry, float rz)
{
	cv::Mat T4x4 = cv::Mat::eye(4, 4, CV_32FC1);
	cv::Mat R3x3_ = cv::Mat::eye(3, 3, CV_32FC1);
	T4x4.at<float>(0, 3) = x;
	T4x4.at<float>(1, 3) = y;
	T4x4.at<float>(2, 3) = z;
	R3x3_ = setR(rx, ry, rz);
	cv::Mat R3x3 = R3x3_;
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			T4x4.at<float>(i, j) = R3x3.at<float>(i, j);
		}
	}
	return T4x4;
}

cv::Mat T_from_q(float x, float y, float z, float qw, float qx, float qy, float qz)
{
	cv::Mat T4x4 = cv::Mat::eye(4, 4, CV_32FC1);
	T4x4.at<float>(0, 3) = x;
	T4x4.at<float>(1, 3) = y;
	T4x4.at<float>(2, 3) = z;

	/* clean and optmize */
	float q[4];
	float M[9];
	q[0] = qx;
	q[1] = qy;
	q[2] = qz;
	q[3] = qw;
	QuatsToMat(q, M);
	cv::Mat R3x3_ = cv::Mat(3, 3, CV_32FC1, M);
	cv::Mat R3x3 = R3x3_;

	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			T4x4.at<float>(i, j) = R3x3.at<float>(i, j);
		}
	}
	return T4x4;
}

float e_dist(cv::Mat T4x4, float gx, float gy, float gz)
{
	float ex = std::abs(T4x4.at<float>(0, 3) - gx);
	float ey = std::abs(T4x4.at<float>(1, 3) - gy);
	float ez = std::abs(T4x4.at<float>(2, 3) - gz);
	float e = sqrt(ex * ex + ey * ey + ez * ez);
	return e;
}

float e_dist_T(cv::Mat T4x4, cv::Mat gtT4x4)
{
	float ex = std::abs(T4x4.at<float>(0, 3) - gtT4x4.at<float>(0, 3));
	float ey = std::abs(T4x4.at<float>(1, 3) - gtT4x4.at<float>(1, 3));
	float ez = std::abs(T4x4.at<float>(2, 3) - gtT4x4.at<float>(2, 3));
	float e = sqrt(ex * ex + ey * ey + ez * ez);
	return e;
}

void QuatsToMat(const float *q, float *m) {
	//x,y,z w
	//try out this implementation: https://www.euclideanspace.com/maths/geometry/rotations/conversions/quaternionToMatrix/index.htm
	float qx = q[0];
	float qy = q[1];
	float qz = q[2];
	float qw = q[3];

	m[0] = 1 - 2 * qy *qy - 2 * qz * qz;
	m[1] = 2 * qx*qy - 2 * qz*qw;
	m[2] = 2 * qx*qz + 2 * qy*qw;
	m[3] = 2 * qx*qy + 2 * qz*qw;
	m[4] = 1 - 2 * qx * qx - 2 * qz * qz;
	m[5] = 2 * qy*qz - 2 * qx*qw;
	m[6] = 2 * qx*qz - 2 * qy*qw;
	m[7] = 2 * qy*qz + 2 * qx*qw;
	m[8] = 1 - 2 * qx * qx - 2 * qy * qy;

}
